...
class PTMDataset(torch.utils.data.Dataset):
    ...
    def collate_fn(self, batch) -> dict[str, Any]:
        ...
        # Pad prot_token_ids, prot_input_mask, and prot_input_mask_no_extra as before
        ...
        # Pad labels properly before converting to tensor
        labels_padded = pad_sequences(labels, constant_value=-1)
        targets = torch.from_numpy(labels_padded)
...
class SingleFastaDataset(PTMDataset):
    def __init__(
        ...
        if dataset_loc.endswith('.fasta') or dataset_loc.endswith('.fa'):
            ...
            filename = f"{dataset_loc}/{train_valid_test}.fasta"
...
# code added by sander:
class AnnotatedDictDataset(PTMDataset):
    def __init__(
        self, 
        ...
        """
        Create a dataset from an annotated sequence dictionary

        Parameters
        ----------
        sequences : Dict[str, str]
            Dictionary mapping protein IDs to their annotated sequences
            Sequences should have '#' markers for target residues
            
        tokenizer : TokenAlphabet
            The tokenizer used for the selected protein representation
        """
        data, unique_prot_ids, num_pos, num_neg = _read_records(sequences)
        super(AnnotatedDictDataset, self).__init__(
            data, unique_prot_ids, tokenizer, num_pos, num_neg
        )
